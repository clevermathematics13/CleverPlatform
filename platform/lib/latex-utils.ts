...
/**
 * Build the approved command-term constraint string for IB_CLASSIFY_SYSTEM.
 * Inlined at module load so the prompt always reflects the canonical list.
 */
const APPROVED_COMMAND_TERMS_BLOCK = (() => {
  const list = DEFAULT_COMMAND_TERMS.map((t) => `  "${t}"`).join(",\n");
  return `The commandTerm field MUST be exactly one of the following approved IB command terms\n(copy the string verbatim — do not combine, abbreviate, or invent variants):\n\n${list}`;
})();
...
Identify the mathematical command term for each part from the question text.

${APPROVED_COMMAND_TERMS_BLOCK}

Common IB phrasings and their correct mapping:
- "Hence show that" → "Hence"  (Hence is the operative term; Show is secondary)
- "Hence or otherwise show that" → "Hence"  (same rule)
- "Show that" → "Show"
- "Prove that" → "Prove"
- "Write down" → "Write down"  (two-word term — use exactly as written)
- "Find" → "Find"
- "Calculate" → "Calculate"
- "Deduce" → "Deduce"
- "Determine" → "Determine"
