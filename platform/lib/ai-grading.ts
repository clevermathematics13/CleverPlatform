...
export function validateGradeResponse(
  ...
  for (const item of parsed.data.items) {
    ...
    // satisfies it is now GRANTED here, not merely flagged. Production
    // evidence: a real result's own note read "8.515 rounds to 8.52, but
    // ... Value is incorrect" -- admitting the match and then withholding
    // the mark anyway -- and this happened repeatedly, with the model
    // inventing a new rationalization each time (wrong s.f. count, a
    // mismatched alternative value, ...) even after flag-only warnings and
    // two rounds of prompt tightening. Flagging alone left the wrong total
    // standing until a teacher happened to notice, so this now corrects it
    // directly, the same way the three withdrawal loops above correct in
    // the opposite direction. This only re-verifies a numeric CLAIM the
    // model itself already made in its own
    // numericCheck/impliedMethodEvidence/intermediateValueCheck field -- it
...
6. For an A mark tied to a specific numerical value where the mark scheme or question states a precision requirement — a final answer, or any step the mark scheme explicitly says must be "correct to N significant figures / decimal places", "given to...", or similar — the student's value must match that required precision exactly. A value that differs — even slightly — is wrong and does not earn the mark, unless the mark scheme itself gives an explicit tolerance or alternative (e.g. "accept 0.946 to 0.948", "465 (or 464 from 3sf)", "or equivalent"). Do not invent your own tolerance for "rounding," "calculator precision," "acceptable rounding," or "close enough" that the mark scheme doesn't state — if the mark scheme is silent on tolerance, there is none. This is separate from rule 5: rule 5 is about the *form* of a correct value (0.5 vs 1/2), this rule is about whether the value itself is the *correct* one. This rule governs a mark with an actual (explicit or default-final-answer) precision requirement; for an INTERMEDIATE working value that the mark scheme merely displays as a reference figure with no such requirement, see rule 15 — do not apply this rule's exactness to every number a mark scheme happens to print.
   ...
   Do NOT withhold the mark by reasoning "the student gave N significant figures/decimal places but M were required" when N is GREATER than M and the value still rounds correctly at M — that is excess precision, which this rule already accepts, and citing the digit count alone as if it were the problem is exactly the same self-invented rejection this rule forbids, just phrased as a digit count instead of "rounding error." Having more digits than required is only ever a problem when the extra-precision value does NOT round to the required one (the earlier case in this rule) — a correct rounding is never made wrong by the number of digits it took to state it. If you find yourself both confirming a value rounds correctly AND withholding its mark in the same breath, that is a contradiction — the mark is due.
...
/**
 * The opaque subject id helpers now live in lib/grading-subject.ts, which has
 * no fs/path imports so a client component can use them too. Re-exported here
 * because every server-side caller already imports them from this module.
 */
export {
