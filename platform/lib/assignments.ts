...
export function buildActivityGeneratorSystemPrompt(
  ...
  return [
    ...
    "11c. COMMAND TERMS MUST BE FROM THE CANONICAL IB LIST ONLY (Calculate, Comment, Compare, Compare and contrast, Construct, Contrast, Deduce, Demonstrate, Describe, Determine, Differentiate, Distinguish, Draw, Estimate, Explain, Find, Hence, Hence or otherwise, Identify, Integrate, Interpret, Investigate, Justify, Label, List, Plot, Predict, Prove, Show, Show that, Sketch, Solve, State, Suggest, Verify, Write down). Do NOT use 'Simplify' — it reads naturally but is not an IB command term; use 'Solve' or 'Calculate' instead for a simplification task (e.g. 'Solve, giving your answer as a fraction in its lowest terms' rather than 'Simplify, giving your answer as a fraction in its lowest terms'). This applies to every question and subpart prompt, not just the first sentence of the packet.",
    `11b. MATH: EVERY mathematical expression in this packet goes inside $...$, in EVERY field of the schema and not only in question prompts — syllabusTopics, prerequisites, materials, atl, compulsoryCore, section headings, spotlight.body, prerequisiteBox items, translationTable rows, geometricReading, tokProvocations, internationalMindedness and reflectionQuestions all included. Whatever sits outside $...$ is printed as the literal characters you typed, so a Syllabus Topics line reading \"ax^2+bx+c\" reaches the student as a, x, a caret and a 2, and a Prerequisites line reading \"a div b := a times 1/b\" reaches them as the English words \"div\" and \"times\" standing where ÷ and × belong. Both are real defects off a printed packet. Write $a x^2 + b x + c$ and $a div b := a times 1/b$ instead; one sentence may mix the two freely, as in \"the standard form $a x^2 + b x + c$, where $a eq.not 0$\". Prose that only names a quantity (\"the leading coefficient\", \"a is not zero\") stays prose — it is symbols that need the delimiters. Write them in native Typst math syntax — NOT LaTeX. No backslash commands (no \\frac, \\sqrt, \\alpha). Use: x^2, x_1, sqrt(x), (a)/(b) or frac(a,b), alpha, beta, sigma, mu, sum_(k=1)^n, integral, macron(x) for a bar/overline. Operators and relations, by their Typst names: times for ×, div for ÷, dot for the dot operator, plus.minus for ±, lt.eq / gt.eq / eq.not for ≤ ≥ ≠ (NEVER leq/geq/neq — those are LaTeX names Typst does not define), degree for the degree sign ° (just degree, NEVER degree.o or degree.circle - those are not names Typst has), in for set membership, RR/ZZ/NN/QQ/CC for the number sets, min/max/mod/det as-is. When you quote a student's incorrect working, do NOT put quotation marks around mathematics that contains $...$ inside it. The quote marks and the dollar signs collide, leaving a string Typst never sees the end of, and the packet fails to render at all. Write it as ordinary prose with the working in math: A student writes $x/4 = 12$, then divides both sides by 4 to get $x = 3$. Identify the error. Never: A student's working is shown: ${q}x/4 = 12$, so divide by $4$: $x = 3.${q} ALWAYS put a space between a function name and what follows it: write $sin x$, $cos 2 theta$, $2 sin theta cos theta$ — NEVER $sinx$, $cos2theta$ or $2sinthetacostheta$. Typst reads an unbroken run of letters and digits as ONE name, so $sinx$ asks for a variable called sinx and $cos2theta$ for one called cos2theta; neither exists, and a single one of them makes the whole packet fail to render. For a limit or a tendency write arrow.r, never the word 'to': $lim_(n arrow.r infinity) u_n = 0$, because Typst has no name 'to' and $n to infinity$ will not typeset. Units go INSIDE the math as quoted text — $1 ${q}m${q}^2 = 10000 ${q}cm${q}^2$ — never as a bare superscript fenced off by dollar signs out in the prose: writing 'cm$^2$ to m$^2$' pairs those dollars around a lone ^2, which is not an expression and also fails the render. Named operators with no Typst symbol (Var, Cov, Corr, SD) MUST be quoted so they render upright and compile correctly, e.g. $${q}Var${q}(X) = sigma^2$ — an unquoted $Var(X)$ will fail to compile. CRITICAL JSON RULE: every ${q} character you write — including these quoted operators — MUST be escaped as \\${q} inside JSON string values, exactly like any other quote mark. A raw, unescaped ${q} inside a string breaks the JSON.`,
    "11d. NEVER write a literal $ for currency in prose (e.g. \"costs $2\", \"a $10 prize\", \"the $2 cost to play\"). The renderer treats every $ as a Typst math-mode delimiter — see rule 11b — with no special case for dollar signs meaning money. A single stray currency $ pairs up with the next $ anywhere else in the packet and gets parsed as a math expression, which reliably breaks the compile (observed failure: \"$2 to play\" parsed as math and crashed on the word \"to\", since Typst tried to evaluate \"2 to play\" as code). For any monetary amount, always spell it out in words instead: \"costs 2 dollars\", \"a 10-dollar prize\", \"a net profit of 3.50 dollars\". This applies everywhere in the packet, not just inside $...$ math — the danger is the bare $ character appearing in prose at all.",
