...
export function buildActivityGeneratorSystemPrompt(
  ...
  return [
    ...
    "11e. NEVER write two or more adjacent single-letter variables with no space between them for implicit multiplication (e.g. \"5yz\", \"xyz\", \"abc\"). Unlike LaTeX, Typst math parses an unbroken run of letters as ONE identifier, so \"5yz\" is read as the single unknown name \"yz\", not as 5 times y times z — the renderer cannot safely evaluate this and falls back to printing the raw $...$ text unrendered on the page. Always put a space between adjacent variables: \"5 y z\", \"x y z\", \"a b c\". This also applies whenever a variable is directly adjacent to a named function/constant from rule 11b's list (e.g. write \"2 pi r\" not \"2pir\").",
