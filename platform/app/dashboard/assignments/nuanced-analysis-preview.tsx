          ...
          <tbody>
            {terms.map((t, i) => (
              <tr key={i} className={i % 2 === 0 ? "bg-white" : "bg-teal-50/60"}>
                <td className="py-1 pr-3 font-bold text-gray-900 whitespace-nowrap align-top w-28 pl-1"><LatexRenderer latex={t.term} /></td>
                <td className="py-1 text-gray-700"><LatexRenderer latex={t.definition} /></td>
              </tr>
            ))}
          </tbody>
        </table>
    ...
    <div className="my-4 rounded-r border-l-4 border-purple-500 bg-purple-50 px-3 py-3">
      ...
      <ol className="list-decimal list-outside ml-4 space-y-2">
        {provocations.map((p, i) => (
          <li key={p.id ?? i} className="text-[10pt] text-gray-800 leading-relaxed"><LatexRenderer latex={p.body} /></li>
        ))}
      </ol>
    </div>
  );
...
function ImBox({ im }: { im: InternationalMindednessBox }) {
  return (
    <div className="my-4 rounded-r border-l-4 border-emerald-500 bg-emerald-50 px-3 py-3">
      ...
      <p className="text-[10pt] text-gray-800 leading-relaxed"><LatexRenderer latex={im.body} /></p>
    </div>
  );
}
    ...
    <div className="mb-3 rounded-r border-l-4 border-amber-400 bg-amber-50 px-3 py-2">
      ...
      <ul className="list-disc list-inside space-y-0.5">
        {box.items.map((item, i) => (
          <li key={i} className="text-[9.5pt] text-gray-700"><LatexRenderer latex={item} /></li>
        ))}
      </ul>
    </div>
  );
...
function SpotlightBlock({ box }: { box: SpotlightBox }) {
  return (
    <div className="mb-2 rounded-r border-l-4 border-blue-400 bg-blue-50 px-3 py-2">
      <p className="text-[9pt] font-bold text-blue-800 mb-0.5"><LatexRenderer latex={box.title} /></p>
      <p className="text-[10pt] text-gray-700"><LatexRenderer latex={box.body} /></p>
    </div>
  );
}
...
function TranslationTableBlock({ table }: { table: TranslationTable }) {
  return (
    <div className="mt-2 overflow-hidden rounded border border-gray-200">
      <p className="bg-gray-100 px-2 py-1 text-[8.5pt] font-semibold text-gray-600"><LatexRenderer latex={table.caption} /></p>
        ...
        <tbody>
          {table.rows.map((r, i) => (
            <tr key={i} className={i % 2 === 0 ? "bg-white" : "bg-gray-50"}>
              <td className="px-2 py-1 border-t border-gray-100 text-gray-700"><LatexRenderer latex={r.informal} /></td>
              <td className="px-2 py-1 border-t border-gray-100 text-gray-700"><LatexRenderer latex={r.formal} /></td>
            </tr>
          ))}
        </tbody>
      </table>
...
function GeometricBlock({ geo }: { geo: GeometricReading }) {
  return (
    <div className="my-2 rounded-r border-l-4 border-indigo-400 bg-indigo-50 px-3 py-2">
      ...
      <p className="text-[10pt] text-gray-800"><LatexRenderer latex={geo.body} /></p>
    </div>
  );
}
        ...
        <div className="flex-1 min-w-0">
          ...
          {q.subparts?.map((sp, si) => (
            <div key={si} className="mt-2 ml-4 flex gap-2 items-start">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex gap-2 items-start">
                  <p className="flex-1 text-[10pt] text-gray-800 leading-relaxed"><LatexRenderer latex={sp.prompt} /></p>
                ...
                {sp.hint && <p className="text-[8.5pt] italic text-gray-400 mt-0.5">Hint: <LatexRenderer latex={sp.hint} /></p>}
              </div>
            </div>
          ))}
          ...
          {formatting.includeAnswerKey && q.answer && (
            <div className="mt-1 rounded border border-green-200 bg-green-50 px-2 py-1">
              ...
              <span className="text-[8.5pt] text-green-700"><LatexRenderer latex={q.answer} /></span>
            </div>
          )}
          ...
          {q.plantedError && (
            <div className="mt-2 rounded border border-rose-200 bg-rose-50 px-2 py-1">
              ...
              <p className="text-[9pt] text-rose-600 mt-0.5"><LatexRenderer latex={q.plantedError} /></p>
            </div>
          )}
        </div>
      </div>
    ...
    >
      ...
      <header className="mb-6">
        ...
        <div className="border-t-2 border-gray-800 pt-2">
          ...
          {nd.syllabusTopics && (
            <p className="text-[9.5pt] mt-1"><strong>Syllabus Topics:</strong> <LatexRenderer latex={nd.syllabusTopics} /></p>
          )}
          {nd.prerequisites && (
            <p className="text-[9.5pt] mt-0.5"><strong>Prerequisites:</strong> <LatexRenderer latex={nd.prerequisites} /></p>
          )}
          {nd.materials && (
            <p className="text-[9.5pt] mt-0.5 italic"><LatexRenderer latex={nd.materials} /></p>
          )}
          {nd.atl && (
            <p className="text-[9.5pt] mt-1 text-gray-600">
              <strong className="text-gray-700">ATL skill:</strong> <em><LatexRenderer latex={nd.atl} /></em>
            </p>
          )}
          {nd.compulsoryCore && (
            <div className="mt-2 rounded-r border-l-4 border-emerald-500 bg-emerald-50 px-3 py-1.5">
              <p className="text-[9pt] font-bold text-emerald-800">
                ...
                <span className="font-normal"><LatexRenderer latex={nd.compulsoryCore} /></span>
              </p>
            </div>
          )}
        </div>
        ...
        <section className="mb-4">
          <ol className="list-decimal list-outside ml-5 space-y-1 text-[10pt] text-gray-800">
            {draft.instructions.map((ins: string, i: number) => (
              <li key={i}><LatexRenderer latex={ins} /></li>
            ))}
          </ol>
        </section>
      )}
