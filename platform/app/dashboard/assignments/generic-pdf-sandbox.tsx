                ...
                {templates.length === 0 ? <p className="text-xs text-da-muted">No saved analyses yet.</p> :
                  templates.map((t) => (
                    <div key={t.id} className="flex items-center gap-1 group">
                      ...
                      <a href={`/dashboard/assignments/editor/${t.id}`}
                        className="shrink-0 rounded border border-indigo-500/30 bg-indigo-500/10 px-1.5 py-1 text-[10px] text-indigo-300 opacity-0 group-hover:opacity-100 transition-opacity hover:bg-indigo-500/20"
