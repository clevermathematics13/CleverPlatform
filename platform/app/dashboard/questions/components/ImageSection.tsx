    ...
    <div className="space-y-2">
      ...
      <div className="flex items-center justify-between flex-wrap gap-2">
        <button type="button" onClick={onToggleParts}
          className="flex items-center gap-1.5 text-xs font-bold text-da-text hover:text-da-text select-none"
          title={partsCollapsed ? "Show question parts" : "Hide question parts"}>
          Images
          <span className="text-[10px] text-da-muted">{partsCollapsed ? "▲" : "▼"}</span>
        </button>
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {hasTroubleshooting && (
            <button type="button" onClick={onCopyTroubleshooting}
              className="rounded border border-da-border bg-da-surface px-2 py-1 text-xs font-semibold text-da-muted hover:bg-da-hover">
      ...
      {/* Tabs row */}
      <div className="flex items-center gap-1 border-b border-da-border">
        {groups.map((g) => (
          <button
            key={g.type}
            type="button"
            ...
            className={`flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded-t-md border border-b-0 transition-colors ${
              activeTab === g.type
                ? g.tabActive + " border-da-border -mb-px"
          ...
          >
            ...
            {g.imgs.length > 0 && (
              <span className={`rounded-full px-1.5 py-0.5 text-[10px] font-bold leading-none ${
                activeTab === g.type ? "bg-da-surface/30" : g.type === "question" ? "bg-indigo-500/15 text-indigo-300" : "bg-emerald-500/15 text-emerald-300"
              }`}>
                {g.imgs.length}
              </span>
            )}
      ...
      <div className="flex gap-3" style={{ height: PANEL_H }}>
        ...
        <div className="overflow-y-auto flex flex-col gap-3 min-w-0" style={{ width: "50%" }}>
          ...
          {imgs.map((img, imgIdx) => (
            <div key={img.id} draggable
              ...
              className={`relative group rounded-xl overflow-hidden border-2 transition-all bg-da-surface shadow-sm shrink-0 ${
                dragOverImageId === img.id ? "border-blue-500 scale-[1.02] cursor-grabbing" : "border-da-border hover:border-blue-400 hover:shadow-xl cursor-pointer"
              }`}
            ...
            >
              ...
              <div className="absolute top-2 left-2 opacity-0 group-hover:opacity-100 transition-opacity">
                <button type="button" disabled={imgIdx === 0}
                  ...
                  title="Move up"
                  className="rounded-full bg-da-surface/90 text-da-text w-8 h-8 text-sm font-bold flex items-center justify-center hover:bg-blue-600 hover:text-white disabled:opacity-0 disabled:pointer-events-none shadow-lg transition-colors">
                  ↑
                </button>
              </div>
          ...
          {imgs.length === 0 ? (
            <div className={`flex flex-col items-center justify-center gap-3 h-full rounded-xl border-2 border-dashed ${emptyBorder} ${emptyBg}`}>
              <p className="text-sm font-semibold text-da-muted">No {label.toLowerCase()} images yet</p>
            ...
            <div className={`shrink-0 rounded-xl border-2 border-dashed ${emptyBorder} ${emptyBg} px-3 py-3 flex items-center gap-2`}>
              <span className="text-xs font-semibold text-da-muted flex-1">Add image</span>
              <button type="button" disabled={uploadingImage}
                onClick={() => handlePaste(type)}
        ...
        {/* LaTeX column */}
        <div className={`overflow-y-auto rounded-xl border ${accentBorder} bg-da-surface shadow-sm flex-1 min-w-0`}>
            ...
            {latex.length === 1 && (
              editingKey === `${type}-${latex[0].partId}` ? (
                ...
                <button
                  ...
                  title="Edit LaTeX"
                  className="shrink-0 rounded px-1.5 py-0.5 text-[10px] font-bold border border-da-border bg-da-surface text-da-muted hover:bg-da-hover hover:text-da-text transition-colors"
                >
                  ✏ Edit
                </button>
              )
          ...
          {(checking || checkError || checkResult) && (
            <div className="border-b border-da-border bg-violet-500/15/40 px-3 py-2.5 space-y-2">
              ...
              {checkResult && (
                <>
                  <div className="flex items-start gap-2">
                    <p className={`flex-1 text-[11px] font-bold ${checkResult.finalMatches ? "text-emerald-300" : "text-amber-300"}`}>
                      ...
                      {checkResult.remainingDiscrepancies.map((d, i) => (
                        <li key={i} className="flex items-start gap-1.5 text-[11px] leading-snug">
                          <span className={`shrink-0 rounded px-1 py-0.5 text-[9px] font-bold uppercase ${
                            d.severity === "high" ? "bg-red-500/15 text-red-300"
                            : d.severity === "medium" ? "bg-amber-500/15 text-amber-300"
                            : "bg-da-hover text-da-muted"}`}>
                            {d.kind.replace(/_/g, " ")}
                          </span>
                          <span className="text-da-text">
                            {d.description}
                            {d.location && <span className="text-da-muted"> — {d.location}</span>}
                          </span>
                        </li>
                      ))}
                    </ul>
                  ...
                  {checkResult.proposedLatex && (
                    <div className="flex items-center gap-2 pt-0.5">
                      ...
                      <button
                        ...
                        className="rounded border border-da-border bg-da-surface px-2 py-0.5 text-[10px] font-semibold text-da-muted hover:bg-da-hover"
                      >
                        Review it first
                      </button>
                    </div>
          ...
          {latex.length > 0 ? (
            <div className="divide-y divide-da-border">
                  ...
                  <div key={partId} className="px-3 py-2.5 space-y-1">
                    ...
                    {isEditing ? (
                      <div className="space-y-2">
                        ...
                        />
                        {editDraft.trim() && (
                          <div className="rounded border border-da-border bg-da-hover px-2 py-1.5 text-sm text-da-text overflow-x-auto">
                            <LatexRenderer latex={editDraft} />
                          </div>
                        )}
                        {saveLatexError && (
                          <p className="text-xs font-semibold text-red-300">{saveLatexError}</p>
                        )}
                        <div className="flex items-center gap-2">
                          ...
                          <button
                            ...
                            disabled={savingLatex}
                            onClick={() => { setEditingKey(null); setSaveLatexError(null); }}
                            className="rounded border border-da-border bg-da-surface px-2.5 py-1 text-xs font-semibold text-da-muted hover:bg-da-hover disabled:opacity-50"
                          >
                            Cancel
                          </button>
                        </div>
                      ...
                      <div className="text-sm leading-relaxed text-da-text overflow-x-auto">
            ...
            <div className="px-4 py-4 space-y-3">
              <p className="text-xs text-da-muted leading-snug">No LaTeX stored. Use the Extract button above to run extraction from the images.</p>
              {convertLatexError && convertingLatex === null && (
                <p className="text-xs text-red-300 font-semibold">{convertLatexError}</p>
              )}
            </div>
          )}
        </div>
      ...
      {!driveConnected && questionImages.length === 0 && msImages.length === 0 && (
        <p className="text-xs text-da-muted italic">Connect Google Drive to extract images from question documents.</p>
      )}
      ...
      {currentLightboxImage && createPortal(
        <div className="fixed inset-0 z-[400] flex items-center justify-center bg-black/85" onClick={closeLightbox}>
          <button type="button" onClick={(e) => { e.stopPropagation(); prevImage(); }} disabled={lightboxIndex === 0}
            className="absolute left-4 top-1/2 -translate-y-1/2 z-10 rounded-full bg-da-surface/20 hover:bg-da-surface/40 disabled:opacity-20 disabled:cursor-not-allowed text-white w-14 h-14 flex items-center justify-center text-3xl font-bold transition-all shadow-xl border border-white/30"
          ...
          <div className="relative flex flex-col items-center" style={{ maxWidth: "88vw", maxHeight: "90vh" }}
            ...
            <img
              ...
              className="rounded-lg shadow-2xl bg-da-surface"
            />
              ...
              <div className="mt-2 flex gap-1.5">
                {allImages.map((_, i) => (
                  <button key={i} type="button" onClick={(e) => { e.stopPropagation(); setLightboxIndex(i); }}
                    className={`rounded-full transition-all ${i === lightboxIndex ? "w-4 h-2.5 bg-da-surface" : "w-2.5 h-2.5 bg-da-surface/40 hover:bg-da-surface/70"}`}
                    title={`Image ${i + 1}`} />
                ))}
              </div>
            )}
          ...
          <button type="button" onClick={(e) => { e.stopPropagation(); nextImage(); }} disabled={lightboxIndex === allImages.length - 1}
            className="absolute right-4 top-1/2 -translate-y-1/2 z-10 rounded-full bg-da-surface/20 hover:bg-da-surface/40 disabled:opacity-20 disabled:cursor-not-allowed text-white w-14 h-14 flex items-center justify-center text-3xl font-bold transition-all shadow-xl border border-white/30"
