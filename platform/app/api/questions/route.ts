...
export async function GET(request: NextRequest) {
  ...
  if (idParam) {
    // Direct lookup by primary key. Used by the Question Studio modal to open a
    // specific question (e.g. from the ExamBuilder queue) by its database id.
    // This intentionally bypasses the text search, list filters, pagination, and
    // the doc/pdf-presence listing filter below — a known id should always resolve
    // regardless of whether the row would otherwise appear in the browsable list.
