...
**General rule:** Ask "Does the advanced subtopic's syllabus description explicitly mention the skill tested?" If yes, tag only the advanced subtopic. Only add the foundational subtopic if the part is **directly** testing foundational knowledge (e.g. the part ONLY asks the student to recall what the two-backslash-escaped log base ten of e equals, not to apply a log law).

**Example:** Part (a) asks the student to show a two-backslash-escaped log identity: 1 plus log base 2 of n equals log base 2 of 2n, using the product rule. Tag **1.7** (log laws) only. Do NOT also tag **1.5** (introduction to logarithms) — the product rule is defined in 1.7, not 1.5.
...
**Worked example:** "Prove that the set u sub n, written with two-backslash-escaped braces, is an arithmetic sequence, stating clearly its common difference."
...
**This is the ONLY exception to the anti-bleed-over rule below.** For 1.15 parts, you MUST read the full question context — including the stem — to identify WHAT is being proved and assign the companion subtopic accordingly. The statement being proved often appears in the stem (e.g. "Seema claims that n is greater than the two-backslash-escaped log base 2 of n") or in an earlier part; you must use that context to identify the companion code.

**1.15 examples (induction structure confirmed in markscheme):**
- "Use induction to prove Seema's claim is valid" where the stem states the claim is n greater than two-backslash-escaped log base 2 of n → **1.15** + **1.7**
- "Prove by induction that the two-backslash-escaped sum from k=1 to n of k squared equals the two-backslash-escaped fraction n(n+1)(2n+1) over 6" → **1.15** + **1.2**
- "Prove by induction that (two-backslash-escaped cos theta plus i two-backslash-escaped sin theta) to the n equals two-backslash-escaped cos n theta plus i two-backslash-escaped sin n theta" → **1.15** + **1.14**
- "Prove by induction that the two-backslash-escaped sum from k=1 to n of r to the k-1 equals the two-backslash-escaped fraction r^n - 1 over r - 1" → **1.15** + **1.3**
- "Prove by induction that $8^n - 1$ is divisible by 7" → **1.15** only (divisibility has no numbered subtopic)
- "Use proof by contradiction to show the two-backslash-escaped square root of 3 is irrational" → **1.15** only (irrationality has no numbered subtopic)

**1.6 examples (direct deduction — do NOT tag 1.15):**
- "Prove that the set u sub n, two-backslash-escaped braces, is an arithmetic sequence" via $d = u_{n+1} - u_n = $ constant → **1.6** + **1.2**
...
When a part asks the student to evaluate or find a composite function of the form open-paren f-inverse two-backslash-escaped circ g close-paren of x or similar, you must check whether $f^{-1}$ was explicitly derived in an earlier part of the same question.
...
**Worked example:** A question has parts (a)(i) "Find f(2)", (a)(ii) "Find $f^{-1}(x)$", (b) "Write down h(x)", (c) "Solve $h^{-1}(x) = -2$", (d) "Given h(x) equals open-paren f-inverse two-backslash-escaped circ g close-paren of x, find $m$ and $c$".
...
**Test:** After the student writes the conjugate root, do they perform any complex arithmetic? If the two-backslash-escaped pm qi terms immediately cancel and the rest is real — it is **2.12**, not 1.12.
...
- *Example:* In open-paren f-inverse two-backslash-escaped circ g close-paren of x, the composition operator is the recipe; $f^{-1}$ and $g$ are ingredients. Primary = **2.5.1** (Composite functions), NOT 2.5.2.
...
- *Example:* If part (a)(ii) asked "Find $f^{-1}(x)$", then part (d) which builds open-paren f-inverse two-backslash-escaped circ g close-paren of x is NOT re-assessing inverse functions — $f^{-1}$ is pre-computed. Do NOT assign 2.5.2 as primary (or at all) for part (d). Primary = **2.5.1** only.
