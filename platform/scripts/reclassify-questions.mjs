...
const IB_CLASSIFY_SYSTEM_TEMPLATE = `You are an expert IBDP Mathematics question analyst.
...
**Worked example:** "Prove that $ZQZQ{u_nZQZQ}$ is an arithmetic sequence, stating clearly its common difference."
...
**This is the ONLY exception to the anti-bleed-over rule below.** For 1.15 parts, you MUST read the full question context — including the stem — to identify WHAT is being proved and assign the companion subtopic accordingly. The statement being proved often appears in the stem (e.g. "Seema claims that $n > ZQZQlog_2 n$") or in an earlier part; you must use that context to identify the companion code.

**1.15 examples (induction structure confirmed in markscheme):**
- "Use induction to prove Seema's claim is valid" where the stem states the claim is $n > ZQZQlog_2 n$ → **1.15** + **1.7**
- "Prove by induction that $ZQZQsum_{k=1}^{n} k^2 = ZQZQfrac{n(n+1)(2n+1)}{6}$" → **1.15** + **1.2**
- "Prove by induction that $(ZQZQcosZQZQtheta + iZQZQsinZQZQtheta)^n = ZQZQcos nZQZQtheta + iZQZQsin nZQZQtheta$" → **1.15** + **1.14**
- "Prove by induction that $ZQZQsum_{k=1}^{n} r^{k-1} = ZQZQfrac{r^n - 1}{r-1}$" → **1.15** + **1.3**
- "Prove by induction that $8^n - 1$ is divisible by 7" → **1.15** only (divisibility has no numbered subtopic)
- "Use proof by contradiction to show $ZQZQsqrt{3}$ is irrational" → **1.15** only (irrationality has no numbered subtopic)

**1.6 examples (direct deduction — do NOT tag 1.15):**
- "Prove that $ZQZQ{u_nZQZQ}$ is an arithmetic sequence" via $d = u_{n+1} - u_n = $ constant → **1.6** + **1.2**
...
When a part asks the student to evaluate or find a composite function of the form $(f^{-1} ZQZQcirc g)(x)$ or similar, you must check whether $f^{-1}$ was explicitly derived in an earlier part of the same question.
...
**Worked example:** A question has parts (a)(i) "Find f(2)", (a)(ii) "Find $f^{-1}(x)$", (b) "Write down h(x)", (c) "Solve $h^{-1}(x) = -2$", (d) "Given $h(x) = (f^{-1} ZQZQcirc g)(x)$, find $m$ and $c$".
...
**Test:** After the student writes the conjugate root, do they perform any complex arithmetic? If the $ZQZQpm qi$ terms immediately cancel and the rest is real — it is **2.12**, not 1.12.
...
- *Example:* In $(f^{-1} ZQZQcirc g)(x)$, the composition operator is the recipe; $f^{-1}$ and $g$ are ingredients. Primary = **2.5.1** (Composite functions), NOT 2.5.2.
...
- *Example:* If part (a)(ii) asked "Find $f^{-1}(x)$", then part (d) which builds $(f^{-1} ZQZQcirc g)(x)$ is NOT re-assessing inverse functions — $f^{-1}$ is pre-computed. Do NOT assign 2.5.2 as primary (or at all) for part (d). Primary = **2.5.1** only.
...
// The template above uses the marker "ZQZQ" in place of literal double-backslash
// sequences (JS-string-escaped backslashes needed for LaTeX commands like \tfrac,
// \log, \sum, \cos, etc. inside the prompt). Substituted here at runtime via
// character codes to avoid any literal backslash-backslash sequence in this
// source file.
const IB_CLASSIFY_SYSTEM = IB_CLASSIFY_SYSTEM_TEMPLATE.replace(
  /ZQZQ/g,
