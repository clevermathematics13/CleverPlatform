...
- Grade 9 STANDARD Level marking policy lives in
  ...
  data on the test (lib/standards-rubric.ts), never code. A strand's
  standards[] codes are validated against real CCSS Math domains by
  lib/ccss-math-codes.ts (not free text anymore); the official source
  document lives in platform/docs/standards/ccss-math/ — see its README.md.
