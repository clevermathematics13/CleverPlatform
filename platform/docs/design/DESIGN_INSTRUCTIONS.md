...
- Provide a vocabulary list in the header block.
...
**Rules:**
- Every generalisation or proof must be preceded by a **numerical warm-up** (specific values, no variables).
...
- **ELL-specific:** Provide the glossary as a translated insert if the student's L1 is known. At minimum, add a one-line plain-English gloss in parentheses after formal command terms on first use.
...
**Rules:**
- Every **proof by induction** question must include a blank template with labelled rows.
- Every **"Show that"** question must include the first line of working if the question is HL-level or above.
- Every **"Sketch"** task must provide a pre-drawn, labelled axis grid with appropriate scale.
- Every **"Describe" or "Explain"** question must provide a sentence starter appropriate to its type.
- Scaffolding must be **opt-in for gifted/twice-exceptional students**: use a fold-under (print) or collapsed detail block (digital).

**Scaffolding hierarchy (choose the minimum level needed):**

| Level | What to provide |
|---|---|
| 0 (no scaffold) | Question only |
| 1 (framing) | Sentence starter |
| 2 (structural) | Labelled template with blank rows |
...
- **Bold the key mathematical object** in every question stem.
...
- **Readability test:** Cover the question and read only: `[command term] + [mathematical object]`. If that is enough to attempt the task, the question passes.
...
| Geometric / diagrammatic | Argand plane diagram |
| Tabular / numeric | Specific cases with computed values |
| Verbal | Plain-language description |

**Reference materials to always include:**
- Powers-of-$i$ cycle card where relevant
- Pre-drawn, labelled axis grids for every sketch task
- Formula booklet page reference for every formula used
...
**Rules:**
- Every **"List" or "Describe what you have learned"** reflection question must include a **concept map template**.
- Every **TOK reflection** must include a position-statement frame and a modelled mentor text.
- Every **"Explain what is gained by two proofs"** question must offer a **bullet-point option** as an alternative to a paragraph.
- **Oral alternatives:** Every reflection question must be flagged as optionally oral.
...
**Rules:**
- The **compulsory core** (★ and ★★) is stated on page 1 as a specific list of question numbers.
- Every **"Describe" or "Sketch"** task must explicitly state annotated diagrams are acceptable.
- Every **"Explain"** task must explicitly state bullet points are acceptable.
- The activity must be submittable in **digital typed format** without loss of meaning.
- **Partial credit policy** must be stated in the Teacher's Companion.

---

## 4. Question-Level Design Checklist

Before finalising any question, run it through this checklist:

```
□ Command term is in the glossary and is bold on first use.
...
□ For proofs: labelled template included.
...
- **Frame positively before presenting the error.**
- **The error must be a single, identifiable, teachable misconception.**
- **The error must be at exactly one line.**
- **Always include part (a): geometric/structural reason the answer is absurd** before part (b): locating the algebraic error.
- After the student solves it, provide a **"Misconception Name"** in the Teacher's Companion.
...
|---|---|---|
| ADHD | Progress tracker + micro-box at Part start | Questions requiring holding more than two prior results simultaneously |
| Dyslexia | Tear-off command-term strip + colour-coded output demand | Dense unbroken paragraphs; multiple instructions in one sentence |
| Autism spectrum | Explicit criteria for all open-ended items; structured sentence starters | Vague verbs without concrete operationalisation; cluttered layouts |
| Dyscalculia | Powers-of-$i$ cycle card; pre-drawn axis grids; colour-coded steps | Requiring students to track cycles from memory across multiple questions |
| Twice exceptional | Opt-in scaffolding (fold-under); extensions as natural next steps | Forcing gifted students through scaffolding they have internalised |
| Slow processing speed | Compulsory core listed on page 1; each Part completable alone | Packets that only make sense front-to-back in one sitting |
| Math anxiety | Low-stakes entry (conjecture before rule); celebrate partial solutions | Commands like "Prove" without scaffold; all-or-nothing questions |
| Motor/dysgraphia | Digital submission option; pre-drawn diagrams; oral alternative | Requiring precise hand-drawn diagrams for marks |
...
|---|---|
| Academic register | Rewrite in active voice; identify the agent explicitly |
| IB command-term vocabulary | Translated or illustrated glossary; demand-scale visual |
| Mathematical English phrase bank | Proof-writing phrase bank for each proof type |
| TOK and reflective metalanguage | Position-statement frame + mentor text |
| Extended written output | Accept annotated diagrams; bullet-point option; oral alternative |
| Interdisciplinary vocabulary | Flag explicitly; plain-language definition at point of use |

---

## 8. Teacher's Companion — Required Sections

1. **Integration Map:** Table mapping every IB element to specific question numbers.
2. **The Model's "Moves," Located:** Bulleted list identifying each pedagogical move and which question it lives in.
3. **Answer Sketches:** Brief correct answers for every question, including all planted-error keys.
4. **Tiered Deadline Guidance:** Which Parts suit a single 50-minute lesson vs take-home.
5. **Compulsory Core List:** Explicit list of question numbers that constitute minimum engagement.
6. **Differentiation Note:** Specific guidance for ELL, neurodivergent, prior knowledge gaps, gifted students.
7. **Design Note (Honesty Section):** Candid note on which IB topic areas are genuinely integrated vs handled at extension level.

---

## 9. What This Packet Is Not

- **Not a worksheet with harder problems.** A Nuanced Analysis has an arc — not a collection of unrelated questions.
- **Not a packet that only makes sense front-to-back.** Every Part must be enterable from a micro-box summary.
- **Not a packet where the extension feels like punishment.** Extensions are natural next questions for curious students.
- **Not a packet where scaffolding is visible to everyone.** Scaffolding is opt-in.
- **Not a packet where the mathematics is a vehicle for a language test.**
- **Not a packet where "integration" is forced.**

---

*End of Design Instructions. Version 1.0.*
