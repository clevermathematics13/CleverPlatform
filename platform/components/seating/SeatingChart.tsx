...
export default function SeatingChart({ seats, assignments, classGroup }: Props) {
  ...
  // Ref for instant drag feedback
  const positionsRef = useRef<Record<string, PodPosition>>({});
  ...
  // Track live drag positions for instant UI
  const livePositions = useRef<Record<string, PodPosition> | null>(null);

  const onPointerDown = useCallback((e: React.PointerEvent, podId: string) => {
    e.preventDefault();
    const pos = positionsRef.current?.[podId] || { x: 0, y: 0 };
    dragRef.current = { podId, startX: e.clientX, startY: e.clientY, origX: pos.x, origY: pos.y };
    livePositions.current = { ...positionsRef.current };
  ...
  const onPointerMove = useCallback((e: React.PointerEvent) => {
    if (!dragRef.current || !livePositions.current) return;
    const { podId, startX, startY, origX, origY } = dragRef.current;
    livePositions.current[podId] = { x: origX + (e.clientX - startX), y: origY + (e.clientY - startY) };
    // Force re-render
    setPositions((prev) => ({ ...prev }));
  }, []);

  const onPointerUp = useCallback(() => {
    if (livePositions.current) {
      setPositions({ ...livePositions.current });
      try { localStorage.setItem(storageKey, JSON.stringify(livePositions.current)); } catch { /* ignore */ }
    }
    dragRef.current = null;
    livePositions.current = null;
  }, [storageKey]);
  ...
  // Use live drag positions if dragging
  const renderPositions = livePositions.current || positions;
    ...
    <div>
      ...
      >
        {sortedPodIds.map((podId) => {
          ...
          const pos = renderPositions[podId] || { x: 0, y: 0 };
